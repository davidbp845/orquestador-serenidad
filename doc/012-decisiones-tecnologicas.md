...
- **Poco probable: un CrewAI-shaped need.** Multi-agente de verdad — no
  "más tools del mismo agente", sino roles genuinamente distintos
  coordinándose — exigiría que el producto se ramifique de una forma que
  hoy no se ve venir en el crecimiento natural de este caso de uso
  (atención al cliente de un negocio). Es la condición menos probable de
  las tres.
- **Aparte, y más barata que las tres anteriores: observabilidad tipo
  LangSmith.** Un panel de tracing/evaluación con historial entre runs
  podría valer la pena antes que cualquiera de los cambios de arquitectura
